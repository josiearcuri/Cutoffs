...
pad= 200                     # dont change
saved_ts = 4              # which time steps centerline will be saved at
...
cut_thresh = 2            #how many cutoffs to simulate, arbitrary if running for time

#Set Result Directory
result_dir = "sample_results/case4/13/" ##change this to wherever you want to save your results

#Load Existing Centerline

filepath =result_dir + "InitialChannel_13.csv"
...
chb.plot_channels2()
#plt.title(str(bump_scale-1)+ " NE magnitude with "+ str(round(kl*(365*24*60*60.0)/100, 3))+ "ch-w/yr MR constant")
plt.savefig(result_dir+"channelfrombendbybend.png", dpi = 500)
plt.show()
"""
chb.MR_time(result_dir+"bendbybendmr.csv")
plt.title(str(bump_scale-1)+ " NE magnitude with "+ str(round(kl*(365*24*60*60.0)/100, 3))+ "ch-w/yr MR constant")
plt.savefig(result_dir+"bendbybendlong.png", dpi = 500)
plt.show()

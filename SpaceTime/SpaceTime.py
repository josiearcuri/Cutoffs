...
class RipleysKEstimator_spacetime:
    ...
    def _weights(self, xi, yi, diff_d, diff_t):
        ...
        min_dt = [np.min((self.t_max - yi[i], yi[i])) for i in range(len(yi))]
        min_dd = [np.min((self.d_max - xi[i], xi[i])) for i in range(len(xi))]

        weights = [np.asarray(min_dt), np.asarray(min_dd)]
        # print(weights)
        # plt.hist(weights)
        # plt.show()
    ...
    def _evaluate(self, data, dist_space, dist_time, mode):
        ...
        if mode == "K_st":
            ...
            xi, deltaspace = self._pairwise_diffs(data[:, 0])
            yi, deltatime = self._pairwise_diffs(data[:, 1])
            min_dt, min_dd = self._weights(xi, yi, deltaspace, deltatime)
            min_dt[min_dt > dist_time] = dist_time
            min_dd[min_dd > dist_space] = dist_space

            weights = (4 * dist_time * dist_space) / (
                (min_dt + dist_time) * (min_dd + dist_space)
            )

            for x in range(len(dist_space)):
                for t in range(len(dist_time)):
                    dt_indicator = (deltatime <= dist_time[t]) & (
                        deltaspace <= dist_space[x]
                    )
    ...
    def mc_env(self, cutoffs, nit, mode, max_search_d, max_search_t, plotornot):
        ...
        for i in range(nit):
            ...
            k_dt = self._evaluate(
                data=z, dist_time=r_time, dist_space=r_space, mode="K_st"
            )
            mc_dt[:, :, i] = k_dt

            k_d, k_t = self._evaluate(
                data=z, dist_time=r_time, dist_space=r_space, mode="K"
            )
            mc_d[:, i] = k_d
            mc_t[:, i] = k_t
        ...
        if mode == "K_st":
            ...
            # K values
            stat_d, stat_t = self._evaluate(
                data=data, dist_time=r_time, dist_space=r_space, mode="K"
            )
            # space-time K
            stat_dt = self._evaluate(
                data=data, dist_time=r_time, dist_space=r_space, mode=mode
            )
            ...
            return [stat_dt, middle_dt, upper_dt, lower_dt]
        else:
    ...
    def plot_st(self, r_space, r_time, normalized, sig_mask, D, lowerlim, upperlim):
        ...
        im = ax.pcolormesh(
            np.swapaxes(normalized, 0, 1),
            cmap=cmap,
            vmin=-np.max(np.abs(lowerlim, upperlim)),
            vmax=np.max(np.abs(lowerlim, upperlim)),
        )
        # plt.pcolor(np.ma.masked_values(np.swapaxes(normalized*sig_mask,0,1),0), edgecolors='k', linewidths=4, alpha=0.)
        im2 = ax.pcolormesh(
            np.ma.masked_values(
                np.swapaxes(normalized * sig_mask, 0, 1)
                / np.swapaxes(normalized * sig_mask, 0, 1),
                0,
            ),
            zorder=2,
        ...
        ax.set_xticks(r_space / self.width - 0.5)
        ax.set_yticks(r_time / self.dt - 0.5)
        ax.set_yticklabels((r_time).astype(int))
        ax.set_xticklabels((r_space / 100).astype(int))  # , rotation='vertical')

        ax.tick_params(
            axis="both", which="major", top=False, bottom=False, left=False, right=False
        )
    ...
    def plot(
        ...
        # plot CSR envelope
        plt.plot(
            r_time, upper_t, color="red", ls=":", linewidth=0.5, label="_nolegend_"
        )
        plt.plot(
            r_time, lower_t, color="red", ls=":", linewidth=0.5, label="_nolegend_"
        )
        plt.plot(r_time, middle_t, color="red", ls=":", linewidth=0.5, label="CSR")
        plt.plot(
            r_time,
            stat_t,
            color="black",

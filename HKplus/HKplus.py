...
def update_progress(progress, start_time):
    ...
    text = "\rPercent: [{0}] {1}% {2}".format(
        "#" * block + "-" * int(round(barLength - block)), int(progress * 100), status
    )
    sys.stdout.write(
        text + "-- %s minute(s) --" % int(round((time.time() - start_time) / 60))
    )
    sys.stdout.flush()
...
class ChannelBelt:
    ...
    def migrate_years(self, nit, saved_ts, deltas, pad, crdist, Cf, kl, dt, dens=1000):
        ...
        for itn in range(nit):  # main loop
            ...
            ne = update_nonlocal_effects(
                ne, s, self.decay_rate, self.bump_scale, cut_dist, cut_len
            )  # update array of ne with last itn's cutoff(s) and decay old ne
            klarray = nominal_rate(
                kl, ne
            )  ## compute array of nominal migration rate in m/s with nonlocal effects accounted for
            curv = compute_curvature(
                x, y
            )  # get curvature of bends before cutoffs happen
            x, y, R1 = migrate_one_step(
                x, y, W, klarray, dt, k, Cf, D, pad, omega, gamma
            )
            x, y, xc, yc, cut_dist, cut_len, ind1, ind2 = cut_off_cutoffs(
                x, y, s, crdist, deltas
            )  # find and execute cutoffs
            x, y, dx, dy, ds, s = resample_centerline(
                x, y, deltas
            )  # resample centerline
            ...
            if len(xc) > 0:  # save cutoff data
                ...
                cutoff = Cutoff(
                    ...
                    W,
                    cut_dist[0],
                    last_cl_time + (itn) * dt / (365 * 24 * 60 * 60.0),
                    cut_len[0],
                    rad,
                    s[-1],
    ...
    def migrate_bendtracking(
        self, saved_ts, deltas, pad, crdist, Cf, kl, dt, dens=1000
    ):
        """function for computing migration rates along channel centerlines and moving them, limited by number of cutoffs the channel experiences
        inputs:
        saved_ts - which time steps will be saved
    ...
    def plot_channels(self):
        ...
        ax.legend(
            frameon=False,
            loc="lower left",
            bbox_to_anchor=(7000 / (xmax + 2000), 0, 0.1, 0.1),
            markerscale=0.1,
        )
        ax.axis("off")
    ...
    def plot_channels2(self, ncutoff, directory):
        ...
        cmap = LinearSegmentedColormap.from_list(
            "custom red", ["#ffffff", "#de2d26"], N=256
        )
        ...
        for t in range(0, len(times)):
            ...
            if t == 0:
                ...
                ne = update_nonlocal_effects(
                    ne, s, decay, bump, np.array(loc), np.array(dist)
                )
                central_node = np.where(x1 >= cx1[0])[0][0]
                upstream_node = np.where(s >= s[central_node] - dist)[0][0]
                downstream_node = np.where(s >= s[central_node] + dist)[0][0]
            ...
            if t in steps:
                ...
                axs[nextind].fill(
                    ...
                    linewidth=0.1,
                    zorder=order,
                    label="year " + str(2 + int((t - 1) / 4)),
                )
                # axs[i].fill(xm[idx,y1,facecolor='r',edgecolor = 'none', alpha = .5,zorder=order, label= '_nolegend_')
                # plt.plot(chx1[central], chy1[chx1<=x1[0]], color = 'k', linewidth = 1, zorder = order)
                axs[nextind].annotate(
                    text="year " + str(1 + int((t - 1) / 4)),
                    xy=(0.9, 0.9),
                    xycoords="axes fraction",
                    fontsize=10,
                )
                axs[nextind].axis("off")
                nemap = axs[nextind].scatter(
                    ne_x[ne_mag > 0],
                    ne_y[ne_mag > 0],
                    s=40,
                    c=ne_mag[ne_mag > 0],
                ...
                axs[nextind].set_xlim(
                    [cx1[0] - dist[0] / 1.85, cx1[0] + dist[0] / 1.85]
                )
...
def generate_initial_channel(W, D, deltas, pad, Length):
    ...
    x = np.linspace(
        0, cl_length + (2 * pad) * deltas, int(cl_length / deltas + (2 * pad)) + 1
    )  # x coordinate
    y = 10.0 * (
        2
        * np.random.random_sample(
            int(cl_length / deltas) + 1,
        )
        - 1
    )
    y = np.hstack(
...
def generate_channel_from_file(filelist, deltas, matlab_corr=-1):
    ...
    points_fitted = np.vstack(
        [
            spl(np.linspace(0, distance[-1], int(distance[-1] / (W // 2))))
            for spl in splines
        ]
    )
...
def compute_migration_rate(pad, ns, ds, alpha, omega, gamma, R0):
    ...
    for i in range(2, pad):
        si2 = np.hstack(
            (
                np.array([0]),
                np.cumsum(np.hstack((ds[i - 1 :: -1], ds[ns - 1 : pad_up : -1]))),
            )
        )
        G = np.exp(
            -alpha * si2
        )  # convolution vector for downstream boundary to wrap around

        R1[i] = omega * R0[i] + gamma * np.sum(
            np.hstack((R0[i::-1], R0[ns - 1 : pad_up : -1])) * G
        ) / np.sum(
            G
        )  # main equation, weighted sum of curvatures upstream from downstream boundary - periodic boundary condition
    #####################################################
...
def cut_off_cutoffs(x, y, s, crdist, deltas):
    ...
    while len(ind1) > 0:
        ...
        dx, dy, ds, s_little = compute_derivatives(
            x[: ind1[0] + 1], y[: ind1[0] + 1]
        )  # compute derivatives upstream of cutoff
        cl_dist.append(s_little[-1])  # cutoff distance downstream
        dx, dy, ds, s_between = compute_derivatives(
            xc[-1], yc[-1]
        )  # compute derivatives along cutoff bend
...
def update_nonlocal_effects(ne, s, decay, scale, cut_dist, cut_len, thresh=0.05):
    ...
    for k in range(len(cut_dist)):  # for each cutoff, add new NE
        ...
        sigma = (
            cut_len[k] * 1.19
        ) / 2  # want 88% of the bump within 1.19*cut_len, rest is negligible
...
def get_radii(c, ind1, ind2, W):
    ...
    max_rad = 1 / (np.max(radii[ind1:ind2]))
...
def plot_distribution(cuts, W, filepath):
    ...
    ax.scatter(x, y, c="black", s=10, alpha=0.8)
...
def segmented_MR(curv, R1, s, n=90):
    ...
    MR = [
        np.percentile(np.abs(R1[idx[i] : idx[i + 1]]), 100) for i in range(len(idx) - 1)
    ]
